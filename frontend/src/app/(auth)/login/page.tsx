...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const response = await axios.post<AuthTokens>(`${process.env.NEXT_PUBLIC_API_URL}/auth/login`, {
        email,
        password,
 ...
 return (
    <main className="flex items-center justify-center min-h-screen bg-gray-100 p-4">
      <div className="w-full max-w-md bg-white p-8 rounded-lg shadow-lg">
        <h2 className="text-3xl font-bold text-gray-800 text-center mb-6">Вхід</h2>
        ...
        <p className="mt-4 text-center text-gray-600">
          Немає акаунту?{' '}
          <Link href="/register" className="text-blue-500 hover:text-blue-600 font-medium">Зареєструватися</Link>
        </p>
      </div>
    </main>
  );
