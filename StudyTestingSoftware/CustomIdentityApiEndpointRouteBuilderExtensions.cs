...
public static class CustomIdentityApiEndpointRouteBuilderExtensions
{
    ...
    public static IEndpointConventionBuilder MapCustomIdentityApi(this IEndpointRouteBuilder endpoints)
    {
        ...
        var userOptions = endpoints.ServiceProvider.GetRequiredService<IOptions<UserDataOptions>>().Value;
            ...
            ([FromBody] RegisterRequestDTO registration, HttpContext context, [FromServices] IServiceProvider sp) =>
        {
            ...
            if (!registration.IsTeacher && !registration.IsStudent)
            {
                return CreateValidationProblem("InvalidRole",
                    "At least one role (teacher or student) must be selected.");
            }
            ...
            var user = new AppUser()
            {
                ...
                Level = 1,
                Experience = 0,
                RequiredExperience = UserEarningsManager.GetRequiredExperience(1),
            };
            ...
            {
                await userManager.AddToRoleAsync(user, AppRolesConstants.TeacherRole);
            }

            if (registration.IsStudent)
            {
                await userManager.AddToRoleAsync(user, AppRolesConstants.StudentRole);
            }
